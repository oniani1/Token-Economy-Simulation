"""
CrowdTrain v4 Simulation Engine — 3-Pillar Behavioral Redesign
================================================================
Replaces v3's mechanical pipeline with behaviorally rich agents:
  Pillar 1 (operators) — personas + learning + decisions + referrals
  Pillar 2 (customers) — first-class agents (segments, Pareto, satisfaction)
  Pillar 3 (macro)     — sentiment HMM + AMM + events + eras

Each pillar can be toggled via params:
  params['operators'] absent -> Pillar 1 falls back to v3 mechanical model
  params['customers'] absent -> Pillar 2 falls back to v3 monolithic S-curve
  params['macro']     absent -> Pillar 3 falls back to v3 naive price model

For ablations:
  v4_baseline      = all 3 sections present
  v4_no_personas   = remove params['operators']
...
# ─── MAIN SIMULATION ──────────────────────────────────────────────────────
def run_simulation_v4(params: Dict, seed: int = RANDOM_SEED) -> Tuple[List[Dict], List[Customer]]:
    """
    v4 main loop. Returns (history, final_customers_state) so caller can compute
    the 4 supplemental metrics (concentration, NRR, etc.) from the customers list.
    """
    random.seed(seed)

    # Detect which pillars are active
    ...
    next_cust_id = 0
    if customers_on:
        customers_list, next_cust_id = spawn_design_partners(0, design_partner_specs)
...
def evaluate_v4(history: List[Dict], customers: List[Customer]) -> Dict:
    """
...
# ─── MONTE CARLO ──────────────────────────────────────────────────────────
def run_monte_carlo_v4(params: Dict, n_runs: int = NUM_MONTE_CARLO_RUNS) -> Dict:
    all_results = []
    final_states = []
    for i in range(n_runs):
        history, customers = run_simulation_v4(params, seed=RANDOM_SEED + i)
...
def print_results_v4(params: Dict, metrics: Dict):
    print()
    print("=" * 78)
    print("CrowdTrain v4 Monte Carlo Results — 3-Pillar Behavioral Edition")
    print("=" * 78)
